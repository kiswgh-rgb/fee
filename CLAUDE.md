...
→ **항목(아침공부/저녁공부/케틀벨)·가중치를 바꾸면 = 전 데이터 삭제.** 의도 없이 절대 건드리지 말 것.

### 예외 — 이름만 바꾸는 리네임은 `ITEM_RENAMES` 로 (v10.8~)
`day:*`의 `checks`는 **인덱스**로 저장된다 → 순서·가중치가 그대로면 이름만 바뀌어도 데이터 의미가 안 깨진다.
그래서 `ITEM_RENAMES = { '옛이름': '새이름' }` 에 한 줄 넣고 `DEFAULT_CONFIG.items`의 이름을 바꾼다.
`loadConfig()`가 **시그니처 비교 *전에*** 저장된 config의 이름을 갈아끼워 초기화 경로를 타지 않는다(복구 import 경로도 동일하게 보호).
- ⭕ 이름만 교체(순서·가중치 유지) → 데이터 보존 + 상단에 `항목 이름 변경 — …` 안내
- ❌ 가중치·순서·개수 변경 → 여전히 전 데이터 삭제(리네임으로 우회 불가)
- 리네임 항목은 과거 기록의 의미도 새 이름으로 읽히게 된다(예: 5~8월 체크는 원래 수면의식 실행분).
...
## 앱 이름/아이콘 (v10.6~)
- `<title>` = `Fee v10.8`, 홈화면 라벨 = `Fee` (`apple-mobile-web-app-title`).
...
# v10.8 — 습관 항목 리네임: 수면의식 → 아침공부

`DEFAULT_CONFIG.items[0].name` 만 교체(가중치 40·순서 1번째 유지) + `ITEM_RENAMES` 마이그레이션 추가.
`day:*` / `legacy:*` / `total_liter` / `study:*` 전부 무손상 — 위 §"예외 — 이름만 바꾸는 리네임" 참조.
헤드리스 검증(기존 사용자·재실행 멱등·신규 설치 3케이스) 통과.
